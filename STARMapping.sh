...
usage() { printf 'STAR Mapping V1.1
	Mapping using STAR is more complicated than it warrants,
...
log() {	printf "STAR settings for $(date):
	Log File:\t${log}
	Input folder:\t${folder}
	Output folder:\t${out}
	Reference:\t${ref}
...
while getopts "i:q:o:r:l:k:n:hd" arg; do
        case $arg in
                i)
...
#	printf "\nMERGED:$merged\nR1:$r1\nR2:$r2\n" #| tee -a $log # Debugging only
	...
	if [ "$merged" != "NA" ] && [ "$r1" != "NA" ] && [ "$r2" != "NA" ]; then
		...
		STARMapping 2> ${out}STARLogs/$sample.log
